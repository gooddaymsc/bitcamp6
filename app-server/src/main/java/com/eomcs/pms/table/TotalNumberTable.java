...
public class TotalNumberTable extends JsonDataTable<Integer> implements DataProcessor {
  ...
  public static final int COMMENT_NUMBER_INDEX = 3;
  ...
  public void execute(Request request, Response response) throws Exception {
    ...
    switch (request.getCommand()) {
      ...
      case "addNumber.Comment" : addNumberComment(request, response); break;
      default :
  ...
  private void addNumberComment(Request request, Response response) throws Exception {
    checkList();
    int no = list.get(COMMENT_NUMBER_INDEX);
    list.set(COMMENT_NUMBER_INDEX, no+1);
    response.setStatus(Response.SUCCESS);
    response.setValue(no);
  }
  ...
  private void checkList() {
    if (list.size()==0) {
      ...
      list.add(COMMENT_NUMBER_INDEX,1);
    }
  }
}
